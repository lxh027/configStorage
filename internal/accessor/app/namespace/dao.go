...
func (dao *Dao) GetUserNamespace(userId int) ([]Namespace, error) {
	var namespaces []Namespace
	err := global.MysqlClient.Model(Namespace{}).
		Select("namespace.id, namespace.name, namespace.raft_id, namespace.user_id, namespace.private_key").
		Joins("user_namespace on namespace.id = user_namespace.namespace_id").
		Where("user_namespace.user_id = ?", userId).
		Find(&namespaces).
		Error
